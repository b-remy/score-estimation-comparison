...
class UncalibratedLangevin(kernel_base.TransitionKernel):
  ...
  def one_step(self, current_state, previous_kernel_results, seed=None):
    with tf.name_scope(mcmc_util.make_name(self.name, 'mala', 'one_step')):
      ...
      # So, I can't just invert it
      H = -jax.hessian(self.target_log_prob_fn)(current_state)
      H_inv = jax.numpy.linalg.pinv(H)
      sqrt_H_inv = jax.numpy.linalg.cholesky(H_inv)
      ...
      H1 = -jax.hessian(self.target_log_prob_fn)(current_state)
      #sqrt_H1 = jax.numpy.linalg.cholesky(H1)

      H2 = -jax.hessian(self.target_log_prob_fn)(next_state_parts[0])
      #sqrt_H2 = jax.numpy.linalg.cholesky(H2)
...
def _euler_method(random_draw_parts,
  ...
  with tf.name_scope(name or 'mala_euler_method'):
    ...
    for random_draw, state, drift, step_size, volatility in zip(
        ...
        volatility_parts):
      print(H_inv, drift)
      print(sqrt_H_inv, drift)
      proposal = state + H_inv.dot(drift) + sqrt_H_inv.dot( (volatility * tf.sqrt(step_size) * random_draw))
