...
from tensorflow_probability.python.internal.backend.jax import deprecation  # pylint: disable=g-direct-tensorflow-importation  # pylint: disable=g-direct-tensorflow-import
...
class ManifoldMetropolisAdjustedLangevinAlgorithm(kernel_base.TransitionKernel):
...
class UncalibratedLangevin(kernel_base.TransitionKernel):
  ...
  @mcmc_util.set_doc(ManifoldMetropolisAdjustedLangevinAlgorithm.one_step.__doc__)
  def one_step(self, current_state, previous_kernel_results, seed=None):
    with tf.name_scope(mcmc_util.make_name(self.name, 'mala', 'one_step')):
      ...
      """Introduce the Hessian here
      """
      delta = 0.1

      g = jax.grad(self.target_log_prob_fn)(current_state)
      G = jax.numpy.diag(jax.numpy.diag(jax.numpy.outer(g, g.T)))
      sqrt_G = jax.numpy.linalg.cholesky(G)

      _n = G.shape[0]
      H = delta * jax.numpy.eye(_n) + sqrt_G

      M = jax.numpy.linalg.pinv(H)
      ...
      g1 = jax.grad(self.target_log_prob_fn)(current_state)
      g2 = jax.grad(self.target_log_prob_fn)(next_state_parts[0])
      ...
      _n = G.shape[0]
      H1 = delta * jax.numpy.eye(_n) + sqrt_G
      ...
      _n = G.shape[0]
      H2 = delta * jax.numpy.eye(_n) + sqrt_G
  ...
  @mcmc_util.set_doc(
      ManifoldMetropolisAdjustedLangevinAlgorithm.bootstrap_results.__doc__)
  def bootstrap_results(self, init_state):
...
def _euler_method(random_draw_parts,
  ...
  with tf.name_scope(name or 'mala_euler_method'):
    ...
    for random_draw, state, drift, step_size, volatility in zip(
      ...
      print(M)
      sqrt_M = jax.numpy.linalg.pinv(M)
      print(sqrt_M, drift)

      proposal = state + M @ drift + volatility * sqrt_M.dot( tf.sqrt(step_size) * random_draw)
...
def _compute_log_acceptance_correction(current_state_parts,
  ...
  with tf.name_scope(name or 'compute_log_acceptance_correction'):
    ...
    for [
      ...
      v = state_diff - current_drift
      proposed_energy = v.T.dot(H.dot(v)) / step_size
      s,logdet = jax.numpy.linalg.slogdet(H)
      ...
      v = state_diff + proposed_drift
      dual_energy = v.T.dot(Hnext.dot(v)) / step_size
      s, logdet = jax.numpy.linalg.slogdet(Hnext)
